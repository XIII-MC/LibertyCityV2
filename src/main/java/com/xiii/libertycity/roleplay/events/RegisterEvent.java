...
public class RegisterEvent implements Listener {
    ...
    //Runs after the rejoin of the registeration to broadcast a welcome message
    @EventHandler
    ...
    public void forChatRegister(AsyncPlayerChatEvent e) {
        ...
        //Pre load data to check if player is registered, if not: cancel his message
        ...
        Bukkit.getScheduler().runTaskAsynchronously(LibertyCity.INSTANCE, () -> {
            ...
            if(data.playerID <= 0) {
                    ...
                    //If we are in waiting for a rpNom confirmation
                        ...
                        if(optYes.contains(e.getMessage())) {
                            ...
                            if (tempName != null) {
                                ...
                                //Tell the player his name has been permanently set
                                e.getPlayer().sendTitle("§4§l§k|||§r §fBonjour §a§l" + data.rpPrenom + " §2§l" + data.rpNom + "§f! §4§l§k|||", "§6§k§l||§r §7Pour finir entez votre §e§nÂge RP§r §6§k§l||", 0, 14000, 0);
                ...
                if(data.rpPrenom != null && data.rpNom != null && data.rpAge == 0) {
                    ...
                    if(isWaitingAge) {
                        ...
                        if(tempAge >= 18 && tempAge <= 90) {
                            ...
                            e.getPlayer().sendMessage("§2§lLiberty§a§lCity §7» §fVous vous appelez §a§l" + data.rpPrenom + " §2§l" + data.rpNom + " §fet vous avez §6" + tempAge +"§f, c'est bien ça ? §7(Oui/Non)");
                            isWaitingAge = false;
                ...
                if(data.rpPrenom != null && data.rpNom != null && data.rpAge >= 18) {
                    ...
                    if(!isWaitingPrenom && !isWaitingNom && !isWaitingAge && tempPrenom != null && tempName != null && tempAge >= 18) {
                    ...
                    } else e.getPlayer().kickPlayer("§cErreur: RegisterEvent.java (281) -> isWaitingPrenom=" + isWaitingPrenom + " isWaitingNom=" + isWaitingNom + " isWaitingAge=" + isWaitingAge + " tempPrenom=" + tempPrenom + "tempName=" + tempName + " tempAge=" + tempAge + " rpPrenom=" + data.rpPrenom + " rpNom=" + data.rpNom + " rpAge=" + data.rpAge);
